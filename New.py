...
def load_company_culture(file_path: str = "Alldata.json") -> Dict:
    try:
        with open(file_path, 'r') as file:
            data = json.load(file)
        # Assuming Alldata.json contains a list of comments with 'content' field
        tone_indicators = {
            "formal_count": 0,
            "informal_count": 0,
            "average_length": 0,
        ...
        for comment in data:
            ...
            # Simple heuristic for tone: presence of formal words vs. colloquial
            if any(word in content.lower() for word in ["please", "kindly", "regards", "thank you"]):
                tone_indicators["formal_count"] += 1
            else:
                tone_indicators["informal_count"] += 1
            # Extract common phrases (simple bigram analysis)
            words = content.split()
        ...
        tone_indicators["average_length"] = total_length / total_comments if total_comments > 0 else 0
        # Get top 5 common phrases
        tone_indicators["common_phrases"] = sorted(phrase_counts.items(), key=lambda x: x[1], reverse=True)[:5]
        return tone_indicators
    except FileNotFoundError:
        print(f"Error: Alldata.json not found at {file_path}")
        return {}
    except json.JSONDecodeError:
        print(f"Error: Invalid JSON format in {file_path}")
        return {}

def generate_structured_comment(prompt: str, company_culture: Dict, task_comments: List[Dict]) -> Dict:
    # Start with "Selam" as per company culture
    greeting = "Selam"
    
    # Determine tone based on company culture
    tone = "formal" if company_culture.get("formal_count", 0) > company_culture.get("informal_count", 0) else "informal"
    
    # Get common phrases for context
    common_phrases = [phrase for phrase, _ in company_culture.get("common_phrases", [])]
    common_phrase_context = f"Use common phrases like: {', '.join(common_phrases)}." if common_phrases else ""
    
    # Prepare comment context safely
    comment_context = "\n".join([
        f"Comment by {c.get('creator', 'N/A')}: {c.get('content', 'N/A')}" 
        for c in task_comments
    ]) or "No comments available."
    
    # Construct prompt for Gemini API
    gemini_prompt = (
        f"Generate a {tone} comment for a Basecamp task starting with '{greeting}'. "
        f"The comment should address the following user prompt: '{prompt}'. "
        f"Align with the company communication style. {common_phrase_context} "
        f"Use the following task comments for context (do not repeat their content): {comment_context}. "
        f"Keep the comment concise, professional, and relevant to the prompt."
    )
    ...
    try:
        ...
        if response.ok:
            result = response.json()
            generated_content = result.get("candidates", [{}])[0].get("content", {}).get("parts", [{}])[0].get("text", "No comment generated")
            
            # Ensure the comment starts with "Selam"
            if not generated_content.lower().startswith("selam"):
                generated_content = f"{greeting}, {generated_content}"
            
            # Add closing based on tone
            #closing = "Thank you for your attention." if tone == "formal" else "Looking forward to your thoughts!"
           # generated_content = f"{generated_content.rstrip('.')}. {closing}"
            ...
            structured_comment = {
                ...
                "creator": "Chatbot",
                "creator_id": "bot_001",  # Replace with actual bot ID
                "id": f"generated_{int(datetime.now().timestamp())}"
            }
            
            print(f"Generated structured comment: {structured_comment['content']}")
            return structured_comment
        else:
            logging.error(f"Gemini API error: {response.status_code} - {response.text}")
            return {
                "content": f"Selam, Failed to generate comment: {response.text}",
                "created_at": datetime.now().isoformat(),
                "creator": "Chatbot",
                "creator_id": "bot_001",
                "id": f"generated_{int(datetime.now().timestamp())}"
        ...
        logging.error(f"Gemini API request failed: {str(e)}")
        return {
            "content": f"Selam, Error generating comment: {str(e)}",
            "created_at": datetime.now().isoformat(),
            "creator": "Chatbot",
            "creator_id": "bot_001",
            "id": f"generated_{int(datetime.now().timestamp())}"
        ...
        if project:
            ...
            if todolist:
                ...
                if task:
                    ...
                    # Display comments
                    if task['comments']:
                        st.write("Comment fetched successfully")
                    else:
                    ...
                    if st.button("Generate Insights"):
                      ...
                      # Generate Structured Comment section
                    st.subheader("Generate Structured Comment")
                    comment_prompt = st.text_input("Enter your prompt for a new comment (e.g., 'Ask why the team is late'):", key="comment_prompt")
                    if st.button("Generate Comment"):
                        with st.spinner("Generating comment..."):
                             company_culture = load_company_culture("Alldata.json")
                             new_comment = generate_structured_comment(comment_prompt, company_culture, task["comments"])
                             st.markdown(f"**Generated Comment**: {new_comment['content']}")
            # Optionally, post new_comment to Basecamp
    elif page == "Settings":        # Example: post_comment(account_id, project_id, task_id, access_token, new_comment['content'])
        st.header("Settings")
