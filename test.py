...
def get_access_token():
    global ACCESS_TOKEN
    # Check if token exists in file or session state
    if st.session_state.get('access_token'):
        ACCESS_TOKEN = st.session_state.access_token
        logging.info("Using access token from session state")
        # Validate token
...
def is_data_up_to_date(account_id: int, access_token: str, existing_data: List[Dict]) -> bool:
    """Check if existing data is up-to-date by comparing project timestamps."""
...
def main():
    ...
    if page == "Dashboard":
        ...
        3. **Fetch Comments**: On the Tasks page, click 'Fetch Comments' to retrieve task comments.
        4. **Generate Insights**: Click 'Generate Insights' to get AI recommendations from the Gemini API.
        5. **Refresh Data**: Use the 'Force Refresh Data' button on the Tasks page to check for updates.
        """)
        ...
        1. Open a browser for Basecamp authorization.
        2. Save the access token to `access_token.json`.
        3. Fetch projects, to-do lists, and tasks, saving them to `AllData.json`.
        """)
