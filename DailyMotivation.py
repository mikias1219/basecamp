...
def schedule_daily_post(scheduler, account_id: int, project_id: int, message_board_id: int, access_token: str, start_time: time, end_time: time, days: List[str]):
    try:
        ...
        for day in days:
            ...
            is_next_day = start_time.hour * 60 + start_time.minute > end_time.hour * 60 + end_time.minute
            day_index = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"].index(day)
            adjusted_day = day
            if is_next_day:
                next_day_index = (day_index + 1) % 7
                adjusted_day = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"][next_day_index]
            scheduler.add_job(
                check_and_post,
                trigger=CronTrigger(
                    hour=hour,
                    minute=minute,
                    day_of_week=adjusted_day,
                    timezone=EAT_TZ
                ),
            ...
            logging.info(f"Scheduled post for {day} (adjusted to {adjusted_day}) at {hour:02d}:{minute:02d} EAT")
            for delay in range(1, 6):
                fallback_minute = (minute + delay) % 60
                fallback_hour = hour + (minute + delay) // 60
                if fallback_hour >= 24:
                    fallback_hour -= 24
                    next_day_index = (day_index + 1) % 7
                    adjusted_day = ["mon", "tue", "wed", "thu", "fri", "sat", "sun"][next_day_index]
                scheduler.add_job(
                    check_and_post_if_missed,
                    trigger=CronTrigger(
                        hour=fallback_hour,
                    ...
                    args=[start_time, account_id, project_id, message_board_id, access_token],
                    id=f'fallback_post_job_{day}_{delay}',
                    replace_existing=True
                )
                logging.info(f"Scheduled fallback post for {day} (adjusted to {adjusted_day}) at {fallback_hour:02d}:{fallback_minute:02d} EAT")
        scheduler.add_job(
            refresh_daily_image,
        ...
        logging.info("Daily scheduler configured to post at start time with fallbacks")
        return True
    except Exception as e:
...
def check_and_post(test_mode: bool = False, account_id: Optional[int] = None, project_id: Optional[int] = None, message_board_id: Optional[int] = None, access_token: Optional[str] = None):
    ...
    if success:
        logging.info(f"Daily post successful at {current_time.strftime('%H:%M:%S')}")
        st.session_state[f"posted_{current_time.date().isoformat()}_{account_id}_{project_id}_{message_board_id}"] = True
    else:
        logging.error(f"Daily post failed at {current_time.strftime('%H:%M:%S')}")
    return success

def check_and_post_if_missed(start_time: time, account_id: int, project_id: int, message_board_id: int, access_token: str):
    current_time = datetime.now(EAT_TZ)
    current_date = current_time.date().isoformat()
    post_key = f"posted_{current_date}_{account_id}_{project_id}_{message_board_id}"
    if post_key in st.session_state and st.session_state[post_key]:
        logging.debug(f"Post already made today ({current_date}) for account_id={account_id}, project_id={project_id}, message_board_id={message_board_id}. Skipping fallback.")
        return False
    current_minutes = current_time.hour * 60 + current_time.minute
    start_minutes = start_time.hour * 60 + start_time.minute
    if start_minutes <= current_minutes <= start_minutes + 5:
        logging.debug(f"Fallback post triggered at {current_time.strftime('%H:%M:%S')} EAT for start_time {start_time.strftime('%H:%M')}")
        return check_and_post(False, account_id, project_id, message_board_id, access_token)
    logging.debug(f"Fallback post not triggered at {current_time.strftime('%H:%M:%S')} EAT; outside 5-minute window for start_time {start_time.strftime('%H:%M')}")
    return False
...
def main():
    try:
        ...
        current_time = datetime.now(EAT_TZ)
        default_start_time = (current_time + timedelta(minutes=2)).time()  # 11:37 PM EAT
        default_end_time = (current_time + timedelta(minutes=32)).time()   # 12:07 AM EAT
                        ...
                        with st.spinner("Starting daily scheduler..."):
                            ...
                            if config_data and selected_days_cron:
                                ...
                                if success:
                                    ...
                                    st.success(f"Daily scheduler started. Posts will occur at {start_time.strftime('%H:%M')} EAT on {', '.join(selected_days)} with fallback posts for up to 5 minutes if delayed. Image refresh at 00:00 EAT.")
                                    logging.info(f"Daily scheduler started at {start_time.strftime('%H:%M')} on {selected_days_cron}")
                                else:
                                    st.error("Failed to start daily scheduler. Check logs for details.")
                            else:
